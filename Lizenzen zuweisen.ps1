...
- Anzeige aller Lizenzen (Belegt / Frei / Gesamt)
- Verständliche Namen via SKU Mapping
- Lizenzzuweisung an User
#>

# -----------------------------
# 1. Module installieren / importieren
# -----------------------------
...
# -----------------------------
# 3. SKU Mapping (deine Liste – gekürzt + wichtigste enthalten)
# -----------------------------
$skuids = @{
    ...
    'ContentExplorer_Standard'='Information Protection and Governance Analytics â Standard';
    'CORTEX'='Viva Topics';
    ...
    'Customer Lockbox for Government'='Lockbox Enterprise GOV';
    ...
    'Dynamics_365_for_Operations_Sandbox_Tier1_SKU_Plan2'='Dyn 365 Ent Ed. – Sandbox Tier 1:Dev & Test';
...
# -----------------------------
# 4. Lizenzübersicht Tenant
# -----------------------------
Write-Host "`n==============================" -ForegroundColor DarkGray
Write-Host "📊 Lizenzübersicht Tenant" -ForegroundColor Cyan
Write-Host "==============================`n" -ForegroundColor DarkGray

$skus = Get-MgSubscribedSku

$skuTable = foreach ($sku in $skus) {

    $displayName = if ($skuIds.ContainsKey($sku.SkuPartNumber)) {
        $skuIds[$sku.SkuPartNumber]
    } else {
        $sku.SkuPartNumber
    }
...
# Index setzen für Auswahl
$i = 1
$skuTable | ForEach-Object { $_.Index = $i; $i++ }

$skuTable | Format-Table Index, Name, Belegt, Gesamt, Frei -AutoSize
...
# 6. Lizenz auswählen
# -----------------------------
$selection = Read-Host "`nWelche Lizenz soll zugewiesen werden? (Index eingeben)"

$selectedSku = $skuTable | Where-Object { $_.Index -eq [int]$selection }
...
# -----------------------------
try {
    Write-Host "`nWeise Lizenz zu: $($selectedSku.Name)" -ForegroundColor Yellow

    Set-MgUserLicense -UserId $user `
        -AddLicenses @{SkuId = ($skus | Where-Object SkuPartNumber -eq $selectedSku.SKU).SkuId } `
        -RemoveLicenses @()

    Write-Host "✅ Lizenz erfolgreich zugewiesen!" -ForegroundColor Green
}
catch {
    Write-Host "❌ Fehler bei Zuweisung: $_" -ForegroundColor Red
}
